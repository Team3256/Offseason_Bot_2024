...
public class RobotContainer {
  ...
  private void configurePivotShooter() {
    ...
    operator
        ...
        .onTrue(
            new SequentialCommandGroup(
                new PrintCommand("pivot shooter do the thing"),
  ...
  private void configureClimb() {
    ...
    // ? -1 : 1) * 6)));
    //        new Trigger(() -> operator.getRawAxis(translationAxis) ==
    // -1).onTrue(climb.goToZeroLeft());
    //        new Trigger(() -> operator.getRawAxis(secondaryAxis) ==
    // -1).onTrue(climb.goToZeroRight());
    //        new Trigger(
